...
if (document.querySelector('.header')) {
	window.addEventListener('resize', function () {
		if (document.documentElement.clientWidth < 540) {
			document.querySelector('.header__profile').append(document.querySelector('.header__profile-progressbar'));
			document.querySelector('.header__profile').append(document.querySelector('.header__btns'));
		}

		if (document.documentElement.clientWidth > 540) {
			document.querySelector('.header__profile-info').append(document.querySelector('.header__profile-progressbar'));
			document.querySelector('.header__profile-info').append(document.querySelector('.header__btns'));
		}
...
if (document.querySelector('.profil')) {
	window.addEventListener('resize', function () {
		if (document.documentElement.clientWidth < 541) {
			document.querySelector('.profil__name-text').prepend(document.querySelector('.profil__progressbar-lvl-img'));
			document.querySelector('.profil__name-text').append(document.querySelector('.profil__progressbar-lvl-num'));
			document.querySelector('.profil__progressbar-row').append(document.querySelector('.profil__progressbar-lvl-progress'));
			document.querySelector('.profil__row').append(document.querySelector('.profil__btns'));

		}

		if (document.documentElement.clientWidth > 541) {
			document.querySelector('.profil__progressbar-lvl').append(document.querySelector('.profil__progressbar-lvl-img'));
			document.querySelector('.profil__progressbar-lvl').append(document.querySelector('.profil__progressbar-lvl-num'));
			document.querySelector('.profil__progressbar-lvl').append(document.querySelector('.profil__progressbar-lvl-progress'));
			document.querySelector('.profil__info').append(document.querySelector('.profil__btns'));
		}
	})
...
const SWIPER_SWIPER = document.querySelectorAll('.swiper');
...
if (document.querySelector('.amount-selection__checkbox')) {
	...
	if ((document.documentElement.clientWidth > 575) && (SWIPER_SLIDE.length > 7)) {
		...
		SWIPER_SWIPER.forEach(function (item) {
			item.classList.remove('swiper');
		});
		SWIPER_WRAPPER.forEach(function (item) {
			item.classList.remove('swiper-wrapper');
			item.classList.add('swiper-wrapper-reset');
		});
		SWIPER_SLIDE.forEach(function (item) {
...
if (document.querySelector('.timer')) {
		...
		document.querySelector('.instruction').addEventListener('click', function () {
			if (repeateOne < 1) {
				timerSecond('#timerOne', function () {
					let btnTimerInstructin = document.querySelector('.modal-manual__btn');
	...
	if (document.querySelector('a[href="#cases"]')) {
		let repeateTwo = 0;
		document.querySelector('a[href="#cases"]').addEventListener('click', function () {
			if (repeateTwo < 1) {
	...
	function sumScore() {
		...
		if (inputValueDinamic.length > 1) {
			...
			if (inputForm.value >= 69 && inputForm.value <= 109) {
				sumCurrent.value = `${parseFloat(inputForm.value * 725 / 1000).toFixed(0)}`;
			}
	
			if (inputForm.value >= 110 && inputForm.value <= 179) {
				sumCurrent.value = `${parseFloat(inputForm.value * 910 / 1000).toFixed(0)}`;
	
			}
	
			if (inputForm.value >= 180 && inputForm.value <= 239) {
				sumCurrent.value = `${parseFloat(inputForm.value * 1389 / 1000).toFixed(0)}`;
	
			}
	
			if (inputForm.value >= 240 && inputForm.value <= 459) {
				sumCurrent.value = `${parseFloat(inputForm.value * 2084 / 1000).toFixed(0)}`;
	
			}
	
			if (inputForm.value >= 460 && inputForm.value <= 1274) {
				sumCurrent.value = `${parseFloat(inputForm.value * 2174 / 1000).toFixed(0)}`;
	
			}
	
			if (inputForm.value >= 1275) {
				sumCurrent.value = `${parseFloat(inputForm.value * 2353 / 1000).toFixed(0)}`;
...
if (document.querySelector('.sound')) {
	...
	sound.addEventListener('click', () => {
		if (sound.getAttribute('data-value') == 'soundOn') {
			soundImg.setAttribute('src', `../img/gallery/sound-off.png`);
			sound.setAttribute('data-value', 'soundOff');
		}
		else if (sound.getAttribute('data-value') == 'soundOff') {
			soundImg.setAttribute('src', `../img/gallery/sound.png`);
...
if (document.querySelector('.choice-object__btn')) {
	...
	document.querySelector('.choice-object__btn').addEventListener('click', copyToClipboard);
}
...
if (document.querySelector('main') && document.querySelector('footer')) {
	function footerHeigth() {
		...
		let containerHeight = document.querySelector('main .container').offsetHeight;

		// console.log(containerHeight)
		console.log('Высота шапки хедера и футера ' + (containerHeight + headerHeight + footerHeight + 50))
...
if (document.querySelector('.online-chat') && window.innerWidth < 991) {
	...
	function heightOnlineChat() {
		setTimeout(function () {
			if (!document.querySelector('.online-chat').classList.contains('online-chat-focus')) {
				let valueMax = ((document.querySelector('.header__logo').offsetHeight + document.querySelector('.roulette__rull').offsetHeight));

				if (document.documentElement.clientWidth < 991) {
					document.querySelector('.online-chat').style.maxHeight = `${window.innerHeight - valueMax - 30}px`
				}

				if (document.documentElement.clientWidth > 540) {
					document.querySelector('.online-chat').style.maxHeight = `${window.innerHeight - valueMax - 70}px`
				}
			}
		}, 100)
	}
	...
	function eventFocusChat() {
		...
		document.querySelector('.online-chat__input').addEventListener('focus', function () {
			document.querySelector('.online-chat').classList.add('online-chat-focus');
			if (document.documentElement.clientWidth < 991) {
		...
		document.querySelector('.online-chat__input').addEventListener('blur', function () {
			document.querySelector('.online-chat').classList.remove('online-chat-focus');
			if (document.documentElement.clientWidth < 991) {
				document.querySelector('.online-chat').style.top = 'unset';
				document.querySelector('.online-chat').style.maxHeight = `${window.innerHeight - valueMax - 40}px`
			}
			if (document.documentElement.clientWidth > 540) {
				document.querySelector('.online-chat').style.top = 'unset';
				document.querySelector('.online-chat').style.maxHeight = `${window.innerHeight - valueMax - 80}px`
			}
		})
	}
	heightOnlineChat();
...
if (document.querySelector('.roulette__table')) {
	...
	inputTable.addEventListener('input', function () {
		...
		if (inputTable.value != '' && inputTable.value > 0 && inputTable.value <= 500000) {
			currentValue.style.color = '#fff';

			currentValue.classList.remove('roulette__current-block_focus');

			if (inputTable.value >= 10) {
				currentValue.innerText = ((inputTable.value + '000').split('.').join('').split(/(?=(?:...)*$)/)).join(' ');
				console.log(inputTable.value)
			}

			else {
				currentValue.innerText = (inputTable.value + '000').split('.').join('');
			}
	...
	btnsArrow.forEach(function (input) {
		input.addEventListener('click', function (event) {
			...
			if (event.target.textContent == '+10K') {
				...
				currentValue.innerText = String(Number(inputTable.value * 1000)).split(/(?=(?:...)*$)/).join(' ');
			}
			...
			if (event.target.textContent == '1/2') {
				...
				console.log(inputTable.value)
				currentValue.innerText = String(Math.round(Number(inputTable.value * 1000))).split(/(?=(?:...)*$)/).join(' ');
			}
			...
			if (event.target.textContent == 'MAX') {
				...
				currentValue.innerText = String(maxNumber * 1000).split(/(?=(?:...)*$)/).join(' ');
			}
